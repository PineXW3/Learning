...
Hashtable * create_hashtable(int capacity) {
    ...
    hashtable->table = (Person_List **) malloc(sizeof(Person_List *) * capacity);
...
void insert_hashtable(Hashtable *hashtable, Person *the_person) {
    if (hashtable == NULL || the_person == NULL) return;

    int index = hash(hashtable, the_person->key); //Get hash value

    Person_List **table = hashtable->table; //Create table for shorter and cleaner typing
    ...
    hashtable->size += insert_linkedlist(table[index], the_person);
}
...
Person * search_hashtable(Hashtable *hashtable, char *key) {
    ...
    Person_List **table = hashtable->table; //for shorter and cleaner typing
    if (table[index] == NULL) return NULL; //The person does not exist
...
void delete_hashtable(Hashtable *hashtable, char *key) {
    ...
    Person_List **table = hashtable->table; //for shorter and cleaner typing
    if (table[index] == NULL) return; //The person does not exist

    hashtable->size -= delete_linkedlist(table[index], key); //Delete the person and return 1 or 0 depends on result
}
...
unsigned int hash(Hashtable *hashtable, const char *key) {
    ...
    int length = strlen(key); //Get length of the key
